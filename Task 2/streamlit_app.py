...
def load_texts_from_upload(upload, raw_paste) -> List[str]:
        ...
        try:
            if upload.type == "text/csv" or str(upload.name).lower().endswith('.csv'):
                ...
                if 'text' in df.columns:
                    ...
                    # if single-column CSV, take first column
                    texts = df.iloc[:,0].astype(str).tolist()
            else:
                content = upload.getvalue().decode('utf-8')
                # split lines and keep non-empty
    ...
    # filter by min tokens heuristics (simple split)
    if min_tokens > 0:
        filtered = []
        for t in texts:
...
with tabs[0]:
    ...
    with col1:
        ...
        # show raw docs with an expander per doc
...
with tabs[1]:
    ...
    if top_bi:
        # top_bi are tuples where first element may be (w1,w2) or ('token', count) depending on pipeline
        # normalize to (w1,w2,count)
        rows = []
        for item in top_bi:
            if isinstance(item[0], tuple):
                ...
                # if returned as ((w1,w2), cnt) this already fits
                try:
                    (w1, w2), cnt = item
                except:
                    continue
